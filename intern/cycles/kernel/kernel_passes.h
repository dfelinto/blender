...
ccl_device_inline void kernel_write_data_passes(KernelGlobals *kg, ccl_global float *buffer, PathRadiance *L,
	ShaderData *sd, int sample, ccl_addr_space PathState *state, float3 throughput, bool force_lwr)
{
#ifdef __PASSES__
	...
	if(kernel_data.film.pass_lwr) {
		if(!(path_flag & PATH_RAY_LWR_PASS_DONE) && (state->roughness > 0.025f || force_lwr)) {
			kernel_write_lwr_float(buffer + kernel_data.film.pass_lwr, sample, L->depth);
			kernel_write_pass_float3(buffer + kernel_data.film.pass_lwr + 2, sample, L->normal);
			kernel_write_pass_float3(buffer + kernel_data.film.pass_lwr + 5, sample, L->color);
...
ccl_device_inline void kernel_write_light_passes(KernelGlobals *kg, ccl_global float *buffer, PathRadiance *L, int sample, float3 L_sum)
{
	...
	if(kernel_data.film.pass_lwr) {
		kernel_write_lwr_float3(buffer + kernel_data.film.pass_lwr + 8, sample, L_sum, 3);
	}
