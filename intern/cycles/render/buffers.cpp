...
{
#ifdef LWR_INFO
	info_buffer = new float[20*params.width*params.height];
#endif
...
#ifdef LWR_INFO
	for(int i = 0; i < 20; i++)
		write_pfm(string_printf("info_%d.pfm", i).c_str(), info_buffer+i, params.width, params.height, 20);
	delete[] info_buffer;
#endif
}
		...
		if(components == 1) {
			...
			if(type == PASS_SHADOW) {
				if(params.lwr_passes) {
					in += params.get_passes_size(true) + 8 - pass_offset;
					FORALL_PIXELS(3) {
