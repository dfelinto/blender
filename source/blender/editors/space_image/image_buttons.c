...
static void uiTemplateViews(uiLayout *layout, PointerRNA *ptr, PointerRNA *stereo3d_format_ptr)
{
	uiLayout *col, *box;
...
void uiTemplateImageViews(uiLayout *layout, PointerRNA *imaptr)
{
	...
	if (ima->type != IMA_TYPE_MULTILAYER) {
		...
		uiTemplateViews(layout, imaptr, &stereo3d_format_ptr);
	}
	else {
		uiTemplateViews(layout, imaptr, NULL);
	}
}
...
void uiTemplateImageFormatViews(uiLayout *layout, PointerRNA *imfptr)
{
	...
	if (imf->imtype != R_IMF_IMTYPE_MULTILAYER) {
		...
		uiTemplateViews(layout, imfptr, &stereo3d_format_ptr);
	}
	else {
		uiTemplateViews(layout, imfptr, NULL);
	}
}
