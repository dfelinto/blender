...
static GPUTexture *create_3DLUT_from_display_settings(GPUFX *fx, Scene *scene)
{
	...
	// if (display_settings->display_device == "sRGB" || display_settings->display_device == "None")
	//strcpy(display_settings_mod->display_device, display_name);
	...
	for (z = 0; z < LUT3D_EDGE_SIZE; z++)
	{
		for (y = 0; y < LUT3D_EDGE_SIZE; y++)
		{
			for (x = 0; x < LUT3D_EDGE_SIZE; x++)
			{
				...
				// lut3d[offset+0] = MIN2(lut3d[offset+0] * 12.92, pow(MAX2(lut3d[offset+0], 0.00313067), 1.0/2.4) * 1.055 - 0.055);
				// lut3d[offset+1] = MIN2(lut3d[offset+1] * 12.92, pow(MAX2(lut3d[offset+1], 0.00313067), 1.0/2.4) * 1.055 - 0.055);
				// lut3d[offset+2] = MIN2(lut3d[offset+2] * 12.92, pow(MAX2(lut3d[offset+2], 0.00313067), 1.0/2.4) * 1.055 - 0.055);
...
bool GPU_fx_do_composite_pass(GPUFX *fx, float projmat[4][4], bool is_persp, struct Scene *scene, struct GPUOffScreen *ofs)
{
	...
	if (fx->effects & GPU_FX_FLAG_COLORMANAGEMENT) {
		...
		if (colormanage_shader) {
			...
			GPU_shader_uniform_vector(colormanage_shader, exposure_uniform, 1, 1, &view_settings->exposure);
			GPU_shader_uniform_vector(colormanage_shader, gamma_uniform, 1, 1, &view_settings->gamma);
