...
void node_bsdf_diffuse(vec4 color, float roughness, vec3 N, vec3 V, vec4 ambient_light, vec4 direct_light, out vec4 result)
{
	...
	V = (gl_ProjectionMatrix[3][3] == 0.0)? -normalize(V): vec3(0.0, 0.0, 1.0);
...
void node_bsdf_diffuse_one_sphere_light(float NL, vec3 N, vec3 L, vec3 V, float light_distance, float light_radius, float roughness, out float diff)
{
	...
	diff = max(illuminance, 0.0) / (4 * M_PI * M_PI * sqrLightRadius);
	diff *=  M_PI * M_PI * M_PI; //Cycles matching for point light
}

void node_bsdf_diffuse_one_sun_light(float NL, vec3 N, vec3 L, vec3 V, float light_distance, float light_radius, float roughness, out float diff)
{
	light_radius = max(light_radius, 0.0001);
	float sqrDist =  light_distance * light_distance;
	float cosTheta = clamp(dot(N, L), -0.999, 0.999) ; 
...
void node_bsdf_diffuse_one_hemi_light(float NL, vec3 N, vec3 L, vec3 V, float light_distance, float roughness, out float diff)
{
	node_bsdf_diffuse_one_sun_light(NL, N, L, V, light_distance, 0.0, roughness, diff);
}

void node_bsdf_diffuse_one_rectangular_light(vec2 scale, mat4 lampmat, vec3 N, vec3 V, vec3 lampco, float light_distance, float areasizex, float areasizey, out float diff)
{
	N = -N;
	...
	if ( dot(V - lampco, lampv) > 0.0 )
	{
		...
		diff = solidAngle * 0.2 * (
			...
			max(0.0, dot( normalize(lampco - V), N) )
		);
	}

	float normalization = 1 / (areasizex * areasizey);

	diff *= normalization;
	diff *=  M_PI * M_PI; //Cycles matching
	//diff = lampco.z;
}


void node_bsdf_glossy(vec4 color, float roughness, vec3 N, vec3 viewvec, vec4 ambient_light, vec4 direct_light, out vec4 result)
{
	result = vec4( (ambient_light.rgb + direct_light.rgb) * color.rgb, 1.0);
}

void node_bsdf_glossy_one_sphere_light(vec3 N, vec3 L, vec3 V, float light_distance, float light_radius, float roughness, out float specfac)
{
...
void node_bsdf_glossy_one_area_light(vec3 N, vec3 L, vec3 V, float light_distance, vec2 scale, mat4 viewinvmat, mat4 lampinvmat, mat4 lampmat, float areasizex, float areasizey, float roughness, out float specfac)
{
#if 0
	N = normalize(N);
	V = (gl_ProjectionMatrix[3][3] == 0.0)? -normalize(V): vec3(0.0, 0.0, 1.0);
	...
	vec3 R = reflect(-V, -N);
	vec3 E = linePlaneIntersect(V, R, L, lampZ);

	float coneAngle = 0.807 * pow(roughness, 0.447097);
	//float coneAngle = 2.0 * sqrt( 2.0 / (glossiness + 2.0) );
	float coneSin = sin(coneAngle);
	float coneProjectRadius =  coneSin * length(E); // TODO Normalize to match circle area
	coneProjectRadius =  0.00001; // TODO Normalize to match circle area

	//Jumping in lamp 2d space
	vec2 projectionCenter = (lampinvmat * vec4(E - L,0.0) ).xy; //OK

	vec2 br = max( projectionCenter - vec2(coneProjectRadius), -vec2( width, height) ); //OK
	vec2 tl = min( projectionCenter + vec2(coneProjectRadius),  vec2( width, height) ); //OK

	float A = max(tl.x - br.x, 0.0) * max(tl.y - br.y, 0.0); //OK
	vec2 mostRepresentativePoint = (tl + br) / 2.0; //OK

	//Getting back to shadingPoint
	L = L - V + (lampmat * vec4(mostRepresentativePoint,0.0,0.0)).xyz; //OK
	...
	float specAngle = clamp(dot(R,lampZ),0.0,1.0);
	float specmask = specAngle>0.0?1.0:0.0;
	specfac *= specmask;

	float normalization = 1 / (areasizex * areasizey);

	specfac *= normalization;
	specfac *= M_PI * M_PI; //Cycles matching
#else
	...
	L = light_distance * L;
	vec3 lampv = 	normalize( vec3(lampmat * vec4(0.0,0.0,1.0,0.0)) ); //lamp projection axis	
	float masking = max(dot( normalize(V - L), lampv), 0.0);
	L = normalize(L);
	//End of masking
...
void node_bsdf_glossy_one_sun_light(vec3 N, vec3 L, vec3 V, float light_distance, float light_radius, float roughness, out float specfac)
{
