...
bool RE_bake_engine(
        ...
        const ScenePassType pass_type, const float matrix[4][4], float result[])
{
	RenderEngineType *type = RE_engines_find(re->r.engine);
	RenderEngine *engine;
	...
	if (type->bake)
		type->bake(engine, re->scene, object, pass_type, object_id, pixel_array, num_pixels, depth, matrix, result);
