...
bool RE_bake_engine(
        ...
        const ScenePassType pass_type, const ScenePassType custom_flag,
        float result[])
{
	RenderEngineType *type = RE_engines_find(re->r.engine);
	RenderEngine *engine;
	...
	if (type->bake)
		type->bake(engine, re->scene, object, pass_type, custom_flag, object_id, pixel_array, num_pixels, depth, result);
